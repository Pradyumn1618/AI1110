...
\begin{enumerate}
	...
	\item Now we make the circuit for shift register using D-Flip flops with 4 bits $Q_0$,$Q_1$,$Q_2$,$Q_3$ and $D_0$,$D_1$,$D_2$,$D_3$ being their inputs.
	
	\item Then we connected XOR gate according to the figure \ref{XOR} 
	\begin{figure}[h]
	...
	\item Then we connected the A,B,C,D of the decoder (7447 IC) with $Q_0$,$Q_1$,$Q_2$,$Q_3$ respectively as per the figure \ref{7447}
	\begin{figure}[h]
	...
	\item Next, we connect the 7-segment display with the dceoder (7447 IC).
